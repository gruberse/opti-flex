...
from app.models.solver.ga.reduction.nsga2.custom.individual import NSGA2Individual
from app.models.solver.ga.reduction.nsga2.custom.utils import fast_non_dominated_sorting

from app.models.solver.ga.reduction.obj import Reduction


class NSGA2BasedReduction(NSGA2basedReductionBase, Reduction):

    def reduce_individuals(self, individuals: List[Individual], n_individuals: int) -> List[NSGA2Individual]:

        if len(individuals[0].fitness_list) < 2:
            raise RuntimeError('nsga2 reduction can only be used for multi-objective optimization')

        survivors: List[NSGA2Individual] = []

        generator_non_dominated_sorting = fast_non_dominated_sorting(individuals)

        while len(survivors) < n_individuals:
            current_front = next(generator_non_dominated_sorting)

            if len(survivors) + len(current_front) <= n_individuals:
                ...
                n_remaining_individuals = n_individuals - len(survivors)
...
def test():
    ...
    survivors = s.reduce_individuals(individuals, 2)
